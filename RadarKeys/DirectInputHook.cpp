...
namespace RadarKeys {
    namespace DirectInputHook {
        ...
        static Poll_t OrigPoll(IDirectInputDevice8* self) {
            void** vtbl = OriginalVTableForDevice(self);
            return vtbl ? reinterpret_cast<Poll_t>(vtbl[kSlotPoll]) : g_origPoll;
        }
        ...
        static void ClassifyPlaystation(IDirectInputDevice8* self) {
            ...
            if (kSlotGetDeviceInfo >= kDeviceVTableSize) {
            ...
            spdlog::info("DirectInputHook: device {:p} PlayStation mapping {} (product:\"{}\", instance:\"{}\", vid:{:04X}, pid:{:04X})",
        ...
        static HRESULT STDMETHODCALLTYPE Hooked_Poll(IDirectInputDevice8* self) {
            Poll_t origPoll = OrigPoll(self);
            return origPoll ? origPoll(self) : E_FAIL;
        }
        ...
        static HRESULT STDMETHODCALLTYPE Hooked_GetDeviceState(IDirectInputDevice8* self,
            ...
            case DeviceKind::Joystick: {
                ...
                ULONGLONG last = lastBlockLog.load(std::memory_order_relaxed);
                if (now - last >= 2000 && lastBlockLog.compare_exchange_strong(last, now)) {
                    spdlog::info("DirectInputHook: joystick device {:p} blocked to game (PlayStation={})",
                        static_cast<void*>(self), info.isPlaystation ? "yes" : "no");
                    spdlog::default_logger()->flush();
                }
                break;
            }
        ...
        static HRESULT WINAPI Hooked_DirectInput8Create(HINSTANCE hinst, DWORD dwVersion,
            ...
            spdlog::info("DirectInputHook: DirectInput8Create called (dwVersion={:08X}, hr={:08X})",
                dwVersion, static_cast<unsigned>(hr));
            spdlog::default_logger()->flush();
        ...
        void Install() {
            ...
            spdlog::info("DirectInputHook: hooked DirectInput8Create in the loaded dinput8.dll");
            spdlog::default_logger()->flush();
        }
        ...
        static void ClassifyOwnedDevice(IDirectInputDevice8* device, DeviceInfo& info) {
            ...
            spdlog::info("DirectInputHook: self-opened device {:p} PlayStation {} (product:\"{}\", vid:{:04X}, pid:{:04X})",
        ...
        static BOOL CALLBACK EnumJoysticksCallback(const DIDEVICEINSTANCE* pdidInstance, VOID* pContext) {
            ...
            if (FAILED(hrFormat)) {
                ...
                spdlog::info("DirectInputHook: \"{}\" accepted only default joystick format (custom hr={:08X})",
        ...
        static void EnsureOwnDirectInput() {
            ...
            spdlog::info("DirectInputHook: opened independent IDirectInput8 for self-driven polling");
            spdlog::default_logger()->flush();
        }
        ...
        void Poll(HWND hwnd) {
            ...
            if (now - g_lastEnumTick >= 2000) {
                ...
                if (FAILED(hrEnum)) {
                    ...
                    spdlog::warn("DirectInputHook: zero DirectInput game controllers reported");
                }
                ...
                for (auto& entry : g_deviceInfo) {
                    ...
                    if (info.isPlaystation != sonyPresent) {
                        ...
                        spdlog::info("DirectInputHook: Steam virtual 360 device {:p} PlayStation {} (Sony pad {})",
